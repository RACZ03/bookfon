...
@Component({
  selector: 'app-add-categories-workshop',
  templateUrl: './add-categories-workshop.component.html',
  styleUrls: ['./add-categories-workshop.component.scss']
})
export class AddCategoriesWorkshopComponent implements OnInit {

  public title: string = 'New Workshop Category';
  ...
  loadDataForm(data: any = null) {
   ...
   this.title = ( data == null || data == undefined ) ? 'New Workshop Category' : 'Update Workshop Category';
...
async addCategories(){
    ...
    let resp = await this.eventWorkshopSvc.postCategory(this.Categoryadd.value);
